...
class Groups(object):
    ...
    def reorder(self, column=None, ascending = False):
        if not column is None:
            self._group_names = self._subgroups[column].sum().sort_values(ascending=ascending).index
        else:
            self._group_names = self._subgroups.indices
        return self
    ...
    def time_series(self, group_name: str, column = 'time') -> None:
        ...
        return group.groupby(pd.Grouper(freq='ME', closed='left', label='left'))[column].sum().reset_index()
    ...
    def plot(self, groups: Iterable[str]=None, plot='time', n=None, title=None):
        ...
        if groups:
            ...
            elif isinstance(group_names, Iterable):
        ...
        for i_group, group_name in enumerate(group_names):
            ...
            if n is None or i_group<n:
                ...
                ax.bar(dates,amounts,width=relativedelta(months=1), bottom = bottom, label=group_name)
        ...
        if not n is None:
            ax.bar(self._ds.dates,sum_other,width=relativedelta(months=1), bottom = bottom_master, label='Other')
        ax.legend()
        if title:
            ax.set_title(title)
        fig.autofmt_xdate()
...
class DataSet(object):
    ...
    def __init__(self, path: str | Path = None, df = None):
        ...
        if path is None and df is None:
            raise ValueError("Neither a path nor a dataframe were given to the constructor.")
        ...
        if path:
            for filename_in in [ filename for filename in os.listdir(path) if filename.endswith('.db')]:
                ...
                event_list = []
                for i_payload,(time_str,payload_str) in enumerate(zip(df_sql.time,df_sql.payload)):
                    ...
                    if 'total_time_spent' in changes.keys():
                        ...
                        event_list.append([event_time,user['name'],project['name'],timedelta_to_hours(delta)])

                df = pd.DataFrame(event_list,columns=['date','dev','project','time'])
        ...
        self.time_t  = self.df.groupby(pd.Grouper(freq='ME', closed='left', label='left'))['time'].sum()
